...
static const KongFrame *ActorFrame(unsigned who, unsigned address,
  ...
  if (s_move.kind && s_move.address == address && s_move.choice == (unsigned)s_choices[who]) {
    unsigned phase = tick - s_move.started;
    unsigned semantic = s_move.kind == 3 ? 38 : s_move.kind == 2 ? 20 : 174;
    if (s_move.kind == 1 && s_move.landed) {
      semantic = 175;
      phase = tick - s_move.landed;
    }
...
static unsigned TeamPhase(unsigned a, unsigned choice, unsigned animation, uint32_t tick) {
  if (s_move.kind == 3 && s_move.address == a) return tick - s_move.started;
...
uint32_t Dkc2KongsGameplay(uint8_t *ram, const uint8_t *rom, size_t rom_size,
  ...
  if (!choice || (s_move.kind && (s_move.address != a || s_move.choice != choice ||
      (s_move.kind == 1 && (carried || (state != 0 && state != 1 && state != 7 && !(choice == 2 && state == 0x16)))) ||
      (s_move.kind == 2 && animation != 20) ||
      (s_move.kind == 3 && (animation != 38 || (!s_move.airborne && carried != Read16(ram + 0x597))))))) {
    if (s_move.kind && getenv("DKC2_KONGS_TRACE"))
      fprintf(stderr, "kong cancel kind=%u choice=%u state=%u animation=%u carried=%04x\n",
              s_move.kind, choice, state, animation, carried);
    memset(&s_move, 0, sizeof s_move);
    if (!choice) return 0;
  }
  ...
  if (s_pack.has_team && animation == 38 && !s_move.kind &&
      ActorAddress(carried) && carried == Read16(ram + 0x597))
    s_move = (KongMove){a, choice, 3, tick, 0, false, false};
  if (s_move.kind == 3) {
    const unsigned phase = tick - s_move.started, release = choice == 1 ? 15 : 18;
    if (pc == 0xb9dcea) {
      ...
      else if (!s_move.airborne && s_move.prepared && phase >= release)
        SeekThrowCallback(ram, rom, a, 0xd8be);
      else if (!s_move.airborne && !s_move.prepared && phase + 1 >= release)
  ...
  if (pc == 0xb9d8ac && animation == 20 && ActorAddress(carried)) {
    s_move = (KongMove){a, choice, 2, tick, 0, false, false};
  } else if (pc == 0xb9d967 && s_move.kind == 2) {
    const unsigned release = choice == 1 ? 22 : 18;
    if (tick - s_move.started < release) return WaitAnimationCallback(ram, a);
    s_move.airborne = true; /* Release consumed; do not seek it again. */
    ...
    if (ActorAddress(carried)) {
      ...
      if (getenv("DKC2_KONGS_TRACE"))
        fprintf(stderr, "kong throw choice=%u phase=%u object=%04x offset=%d,%d\n", choice,
                tick - s_move.started, carried, (int16_t)Read16(ram + 0xd7c), (int16_t)Read16(ram + 0xd7e));
    }
  } else if (pc == 0xb9d9e0 && s_move.kind == 2 && ActorAddress(carried)) {
  ...
  } else if (pc == 0xb9dfd5 && s_move.kind == 2) {
    if (tick - s_move.started < SequenceDuration(choice, 20))
      return WaitAnimationCallback(ram, a);
    memset(&s_move, 0, sizeof s_move);
  } else if (pc == 0xb89616) {
    if (s_move.kind == 2) {
      const unsigned phase = tick - s_move.started;
      if (phase >= SequenceDuration(choice, 20))
        SeekThrowCallback(ram, rom, a, 0xdfd5);
      else if (!s_move.airborne && phase >= (choice == 1 ? 22u : 18u))
        SeekThrowCallback(ram, rom, a, 0xd967);
      return 0;
    }
    if (!s_move.kind && (state == 0 || state == 7 || state == 2) && !carried &&
        !Read16(ram + a + 0xe) && (Read16(ram + 0x981) & 0x400) &&
        (Read16(ram + 0x983) & 0x4000)) {
      s_move = (KongMove){a, choice, 1, tick, 0, false, false};
    ...
    if (s_move.kind != 1) return 0;
    const unsigned phase = tick - s_move.started;
    if ((choice == 1 && phase >= SequenceDuration(choice, 174)) ||
        (s_move.landed && tick - s_move.landed >= 18) || phase > 180) {
      memset(&s_move, 0, sizeof s_move);
    ...
    if (choice == 1) {
      if (Read16(ram + a + 0xe)) { memset(&s_move, 0, sizeof s_move); return 0; }
      if (phase == 18 || phase == 26 || phase == 32) GroundImpact(ram, rom, a, choice, true);
    } else if (!s_move.landed) {
      if (Read16(ram + a + 0xe)) s_move.airborne = true;
      if (phase >= 8 && s_move.airborne && !Read16(ram + a + 0xe) &&
