...
typedef struct CrtPreset {
  const char *name;
  Dkc2CrtSettings values;
} CrtPreset;

static const CrtPreset kPresets[kDkc2CrtPresetCount] = {
    {"living-room",
     {kDkc2CrtPresetLivingRoom, 55, 80, kDkc2CrtMaskGrilleFine, 30, 40, 25,
      50}},
    {"studio",
     {kDkc2CrtPresetStudio, 85, 95, kDkc2CrtMaskGrilleFine, 40, 25, 15, 0}},
    {"soft",
     {kDkc2CrtPresetSoft, 35, 50, kDkc2CrtMaskGrilleCoarse, 20, 60, 40, 67}},
    {"custom", {kDkc2CrtPresetCustom, 55, 80, kDkc2CrtMaskGrilleFine, 30, 40,
                25, 50}},
};
...
void Dkc2CrtSettingsDefault(Dkc2CrtSettings *settings) {
  if (!settings) return;
  *settings = kPresets[kDkc2CrtPresetLivingRoom].values;
}

bool Dkc2CrtSettingsApplyPreset(Dkc2CrtSettings *settings, int preset) {
  if (!settings || preset < 0 || preset >= kDkc2CrtPresetCustom)
    return false;
...
void Dkc2CrtSettingsClamp(Dkc2CrtSettings *settings) {
  if (!settings) return;
  settings->preset = ClampInt(settings->preset, 0, kDkc2CrtPresetCount - 1);
  settings->scanlines = ClampInt(settings->scanlines, 0, 100);
  settings->sharpness = ClampInt(settings->sharpness, 0, 100);
  settings->mask = ClampInt(settings->mask, 0, kDkc2CrtMaskCount - 1);
...
static float MaskPitch(int mask) {
  switch (mask) {
    case kDkc2CrtMaskGrilleFine: return 3.0f;
    case kDkc2CrtMaskGrilleCoarse: return 6.0f;
    case kDkc2CrtMaskSlot: return 6.0f;
    default: return 0.0f;
  }
}
...
bool Dkc2CrtDerive(const Dkc2CrtSettings *input, int viewport_width,
  ...
  if (!input || !params || viewport_width <= 0 || viewport_height <= 0 ||
    ...
    return false;
  Dkc2CrtSettings settings = *input;
  Dkc2CrtSettingsClamp(&settings);
  ...
  if (settings.mask == kDkc2CrtMaskNone || params->mask_strength < 0.005f) {
    params->mask = kDkc2CrtMaskNone;
    params->mask_pitch = 0.0f;
    params->mask_strength = 0.0f;
  }
  params->mask_gain = Dkc2CrtMaskGain(params->mask, params->mask_strength);
...
const char *Dkc2CrtDisplayName(int display) {
  return display == kDkc2DisplayCrt ? "crt" : "flat";
}
...
const char *Dkc2CrtPresetName(int preset) {
  if (preset < 0 || preset >= kDkc2CrtPresetCount) return "custom";
...
static const char *const kMaskNames[kDkc2CrtMaskCount] = {
    "none", "grille", "grille-coarse", "slot"};

const char *Dkc2CrtMaskName(int mask) {
  if (mask < 0 || mask >= kDkc2CrtMaskCount) return "none";
