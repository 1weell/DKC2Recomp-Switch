...
enum { kRom = 0x2E02D1, kSpc = 0x7A9, kLength = 0x32 };
...
int main(void) {
  for (unsigned sfx_mask = 0; sfx_mask <= 255; sfx_mask++) {
    ...
    CHECK(Dkc2RestoreSpcMusicPolicy(&apu, rom, sizeof rom));
  ...
  Setup();
  CHECK(!Dkc2RestoreSpcMusicPolicy(NULL, rom, sizeof rom));
  CHECK(!Dkc2RestoreSpcMusicPolicy(&apu, NULL, sizeof rom));
  CHECK(!Dkc2RestoreSpcMusicPolicy(&apu, rom, sizeof rom - 1));
  apu.ram[kSpc + 7] ^= 1;
  CHECK(!Dkc2RestoreSpcMusicPolicy(&apu, rom, sizeof rom));
  ...
  CHECK(!Dkc2RestoreSpcMusicPolicy(&apu, rom, sizeof rom));
  CHECK(apu.ram[kSpc + 2] == 0xD0 && dsp.channel[0].gain == 1000);
  puts("dkc2 SPC music policy tests passed");
  return 0;
}
