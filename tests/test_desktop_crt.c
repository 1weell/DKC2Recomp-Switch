...
static void CheckBeamConservation(void) {
  ...
  for (size_t w = 0; w < sizeof widths / sizeof widths[0]; w++) {
    ...
    for (size_t l = 0; l < sizeof levels / sizeof levels[0]; l++) {
      ...
      for (int s = 0; s < kSamples; s++) {
        ...
        const float v = Dkc2CrtBeamSample(lines, kLines, y, sigma_dark,
                                          sigma_bright);
        sum += v;
        if (v > peak) peak = v;
      }
  ...
  const float middle = Dkc2CrtBeamSample(lines, kLines, 30.5f, 0.28f, 0.5f);
  ...
  const float dark_peak = Dkc2CrtBeamSample(lines, kLines, 30.5f, 0.28f, 0.5f);
  const float dark_gap = Dkc2CrtBeamSample(lines, kLines, 31.0f, 0.28f, 0.5f);
  for (int i = 0; i < kLines; i++) lines[i] = 1.0f;
  const float white_peak = Dkc2CrtBeamSample(lines, kLines, 30.5f, 0.28f, 0.5f);
  const float white_gap = Dkc2CrtBeamSample(lines, kLines, 31.0f, 0.28f, 0.5f);
...
static void CheckMaskAndKnee(void) {
  if (fabsf(Dkc2CrtMaskGain(kDkc2CrtMaskNone, 1.0f) - 1.0f) > 1e-6f)
    Fail("no mask has gain");
  /* Grille at 0.3: mean transmission 0.8, gain 1.25. */
  if (fabsf(Dkc2CrtMaskGain(kDkc2CrtMaskGrilleFine, 0.3f) - 1.25f) > 1e-5f)
    Fail("grille gain at 0.3");
  /* The gain restores the mean of a white field over one triad. */
  const float m = 0.3f;
  const float gain = Dkc2CrtMaskGain(kDkc2CrtMaskGrilleFine, m);
  const float mean = gain * (1.0f + 2.0f * (1.0f - m)) / 3.0f;
  if (fabsf(mean - 1.0f) > 1e-5f) Fail("grille gain does not conserve");
  if (Dkc2CrtMaskGain(kDkc2CrtMaskSlot, 0.3f) <= gain)
    Fail("slot mask gain is not above the grille gain");
  /* The knee passes values below it, bends the rest under 1. */
  if (Dkc2CrtSoftKnee(0.5f, 0.9f) != 0.5f) Fail("knee changed a low value");
  if (Dkc2CrtSoftKnee(1.27f, 0.9f) > 1.0f) Fail("knee exceeds 1");
  if (Dkc2CrtSoftKnee(1.27f, 0.9f) < 0.99f) Fail("knee crushes a white stripe");
  if (Dkc2CrtSoftKnee(0.95f, 0.9f) <= 0.9f ||
      Dkc2CrtSoftKnee(0.95f, 0.9f) >= 0.95f)
    Fail("knee is not monotone just above the knee");
}

static void CheckDerive(void) {
  Dkc2CrtSettings settings;
  Dkc2CrtSettingsDefault(&settings);
  Dkc2CrtFrameParams full;
  /* The 16-inch panel at 16:9: 3456x1940 for 342x224. */
  if (!Dkc2CrtDerive(&settings, 3456, 1940, 342, 224, &full))
    Fail("derive rejected the fullscreen viewport");
  if (full.beam_fade != 1.0f) Fail("beam not fully on at fullscreen");
  if (full.mask != kDkc2CrtMaskGrilleFine || full.mask_pitch != 3.0f)
  ...
  Dkc2CrtFrameParams tiny;
  if (!Dkc2CrtDerive(&settings, 298, 224, 256, 224, &tiny))
    Fail("derive rejected the 1x viewport");
  if (tiny.beam_fade != 0.0f) Fail("beam did not fade out at 1x");
  if (tiny.mask != kDkc2CrtMaskNone || tiny.mask_gain != 1.0f)
  ...
  Dkc2CrtFrameParams small;
  if (!Dkc2CrtDerive(&settings, 1194, 896, 256, 224, &small))
  ...
  if (small.mask != kDkc2CrtMaskNone) Fail("mask still on at 4.7 px per pixel");

  /* Studio preset: flat geometry. */
  Dkc2CrtSettings studio;
  if (!Dkc2CrtSettingsApplyPreset(&studio, kDkc2CrtPresetStudio))
    Fail("studio preset");
  Dkc2CrtFrameParams flat;
  if (!Dkc2CrtDerive(&studio, 3456, 1940, 342, 224, &flat))
    Fail("derive rejected the studio preset");
  if (flat.curvature_x != 0.0f || flat.corner_radius != 0.0f ||
      flat.vignette != 0.0f)
    Fail("studio preset has geometry");
  if (Dkc2CrtSettingsApplyPreset(&studio, kDkc2CrtPresetCustom))
    Fail("custom is not a preset");
  if (Dkc2CrtDerive(&settings, 0, 1940, 342, 224, &flat) ||
      Dkc2CrtDerive(&settings, 3456, 1940, 342, 0, &flat) ||
      Dkc2CrtDerive(NULL, 3456, 1940, 342, 224, &flat))
    Fail("invalid derive input accepted");

  /* Out-of-range settings are clamped, not rejected. */
  Dkc2CrtSettings wild = {99, 250, -5, 42, 400, -1, 1000, 77};
  Dkc2CrtSettingsClamp(&wild);
  if (wild.preset != kDkc2CrtPresetCustom || wild.scanlines != 100 ||
      wild.sharpness != 0 || wild.mask != kDkc2CrtMaskSlot ||
...
static void CheckNames(void) {
  int value = -1;
  if (strcmp(Dkc2CrtDisplayName(kDkc2DisplayCrt), "crt") != 0 ||
      strcmp(Dkc2CrtDisplayName(kDkc2DisplayFlat), "flat") != 0 ||
      !Dkc2CrtDisplayFromName("crt", &value) || value != kDkc2DisplayCrt ||
      !Dkc2CrtDisplayFromName("flat", &value) || value != kDkc2DisplayFlat ||
      !Dkc2CrtDisplayFromName("1", &value) || value != kDkc2DisplayCrt ||
      Dkc2CrtDisplayFromName("plasma", &value) ||
      Dkc2CrtDisplayFromName(NULL, &value))
    Fail("display names");
  for (int i = 0; i < kDkc2CrtPresetCount; i++) {
    if (!Dkc2CrtPresetFromName(Dkc2CrtPresetName(i), &value) || value != i)
      Fail("preset name round trip");
  }
  if (!Dkc2CrtPresetFromName("2", &value) || value != kDkc2CrtPresetSoft ||
      Dkc2CrtPresetFromName("9", &value) || Dkc2CrtPresetFromName("", &value))
    Fail("preset numeric names");
  for (int i = 0; i < kDkc2CrtMaskCount; i++) {
    if (!Dkc2CrtMaskFromName(Dkc2CrtMaskName(i), &value) || value != i)
      Fail("mask name round trip");
  }
  if (Dkc2CrtMaskFromName("shadow", &value)) Fail("unknown mask accepted");
}
