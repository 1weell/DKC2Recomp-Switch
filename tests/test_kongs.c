...
static void MovesTests(uint8_t *ram, uint8_t *rom) {
  ...
  for (unsigned choice = 1; choice <= 2; ++choice) {
    ...
    for (unsigned t = 1; t < release; ++t) {
      ...
      // TEAM mode dispatches the other original slot in the same frame.
      // Its idle update must not cancel this actor's throw windup.
      const unsigned partner = 0xe40;
      W16(ram + partner, 0xe8);
      W16(ram + partner + 0x36, 0xa4);
      Dkc2KongsSetChoice(1, 2);
      W16(ram + 0x593, partner); W16(ram + 0x64, partner);
      W16(ram + 0xd7a, 0);
      CHECK(!Dkc2KongsGameplay(ram, rom, 0x400000, 0xb89616, t));
      W16(ram + 0x593, a); W16(ram + 0x64, a);
      W16(ram + 0xd7a, enemy);
    }
